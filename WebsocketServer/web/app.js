new Vue({
    ...
    methods: {
        ...
        this.ws.addEventListener('message', function(e) {
            ...
            if (msg != null) {
                ...
                if (msg.message) {
                    ...
                    var isAIMessage = msgObj.userphone === '豆包AI' || msgObj.userphone.indexOf('AI') !== -1;

                    // 如果收到AI回复，停止思考状态
                    if (isAIMessage && self.aiThinking) {
                        self.stopAIThinking();
                        // 移除思考提示（这里简化处理，实际项目中可能需要更复杂的DOM操作）
                    }

                    if (msgObj.filelink == "" || !msgObj.filelink) {
                        var messageClass = isAIMessage ? 'ai-message' : '';
                        var userDisplay = isAIMessage ?
                            '<i class="material-icons" style="font-size: 16px; vertical-align: middle;">smart_toy</i> ' + msgObj.userphone :
                            msgObj.userphone;

                        self.chatContent += '<div class="chip ' + messageClass + '">' + userDisplay + '</div>'
                        ...
                        self.chatContent += '<div class="chip ' + messageClass + '">'
                                + userDisplay + '</div>'
        ...
        send: function () {
            ...
            if (this.newMsg != '') {
                var message = $('<p>').html(this.newMsg).text(); // Strip out html

                // 检查是否为AI对话指令
                if (this.isAIMessage(message)) {
                    this.sendAIMessageFromInput(message);
                } else {
                this.ws.send(
                    JSON.stringify({
                        message: {
        ...
        // 检查是否为AI消息
        isAIMessage: function(message) {
            return message.startsWith('@ai ') || message.startsWith('/ai ');
        },

        // 从输入框发送AI消息
        sendAIMessageFromInput: function(message) {
            var aiMessage = message.replace(/^(@ai |\/ai )/, '');
            this.sendAIMessageDirect(aiMessage);
        },

        // 直接发送AI消息（从按钮点击）
        sendAIMessage: function() {
            if (this.newMsg.trim() === '') {
                Materialize.toast('请输入要询问AI的问题', 2000);
                return;
            }
            this.sendAIMessageDirect(this.newMsg.trim());
            this.newMsg = '';
        },

        // 发送AI消息的核心逻辑
        sendAIMessageDirect: function(message) {
            if (this.aiThinking) {
                Materialize.toast('AI正在思考中，请稍等...', 2000);
                return;
            }

            // 显示AI思考状态
            this.aiThinking = true;
            this.aiLastMessage = message;

            // 显示用户消息
            var curTime = this.getCurrentTime();
            this.chatContent += '<div class="chip">网页-' + this.username + '</div>'
                + emojione.toImage(message) + '<br/>'
                + '<div class="time" style="color:gray">' + curTime + '</div>';

            // 显示AI思考提示
            this.chatContent += '<div class="ai-thinking">豆包AI正在思考<span class="ai-typing">...</span></div>';

            // 发送AI指令到WebSocket
            this.ws.send('@ai ' + message);
